...
install_directory() {
    ...
    BRANCH_DIR="/home/$USER/ot-node-8-release-testnet"
    ...
    perform_step sudo rm *.zip "Removing zip file"
    OTNODE_VERSION=$(jq -r '.version' $BRANCH_DIR/package.json)
    perform_step sudo mkdir $OTNODE_DIR "Creating new ot-node directory"
    perform_step sudo mkdir $OTNODE_DIR/$OTNODE_VERSION "Creating new ot-node version directory"
    perform_step sudo mv $BRANCH_DIR/* $OTNODE_DIR/$OTNODE_VERSION/ "Moving downloaded node files to ot-node version directory"
    OUTPUT=$(mv $BRANCH_DIR/.* $OTNODE_DIR/$OTNODE_VERSION/ 2>&1)
    perform_step sudo rm -rf $BRANCH_DIR "Removing old directories"
    perform_step sudo ln -sfn $OTNODE_DIR/$OTNODE_VERSION $OTNODE_DIR/current "Creating symlink from $OTNODE_DIR/$OTNODE_VERSION to $OTNODE_DIR/current"
}
...
install_prereqs() {
    ...
    perform_step sudo rm -rf /var/lib/dpkg/lock-frontend "Removing any frontend locks" > /dev/null 2>&1
    perform_step sudo apt update "Updating Ubuntu package repository" > /dev/null 2>&1
    perform_step sudo apt upgrade -y "Updating Ubuntu to the latest version" > /dev/null 2>&1
    perform_step sudo apt install unzip jq -y "Installing unzip, jq" > /dev/null 2>&1
    perform_step sudo apt install default-jre -y "Installing default-jre" > /dev/null 2>&1
    perform_step sudo apt install build-essential -y "Installing build-essential" > /dev/null 2>&1
...
install_fuseki() {
    ...
    perform_step rm /home/$USER/$FUSEKI_VER.zip "Removing Fuseki zip file"
    perform_step mkdir /home/$USER/ot-node/fuseki "Making /home/$USER/ot-node/fuseki directory"
    perform_step cp -r /home/$USER/$FUSEKI_VER/webapp/ /home/$USER/ot-node/fuseki/ "Copying Fuseki files to $OTNODE_DIR/fuseki/"
    perform_step rm -r /home/$USER/$FUSEKI_VER "Removing the remaining /home/$USER/$FUSEKI_VER directory"
...
install_blazegraph() {
    ...
    sudo mv blazegraph.jar $OTNODE_DIR/../blazegraph.jar
    sudo systemctl daemon-reload
...
install_sql() {
    ...
    if [[ -d "/var/lib/mysql/operationaldb/" ]]; then
        ...
        case "$choice" in
            [yY]* ) perform_step $(MYSQL_PWD=$oldpassword $sql -u root -e "DROP DATABASE IF EXISTS operationaldb;") "Overwriting sql repository";;
            [eE]* ) text_color $RED"Installer stopped by user"; exit;;
...
sudo bash -c 'cat <<EOL > /etc/systemd/system/otnode-logger.service
[Unit]
...
sudo systemctl daemon-reload
sudo systemctl enable otnode-logger.service
sudo systemctl start otnode-logger.service
